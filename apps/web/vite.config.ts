...
import { paraglideVitePlugin } from "@saasweave/i18n/vite/plugin";
...
const IS_TEST =
  ...
  process.argv.some((argument) => argument === "test" || argument.endsWith("/vitest"));
...
export default defineConfig({
  ...
  test: {
    include:
      process.env.VITEST_E2E === "1"
        ? ["**/__e2e__/**/*.spec.ts", "!**/__e2e__/playwright/**"]
        : ["**/__tests__/**/*.test.ts"]
  },
  run: {
    tasks: {
  ...
  define: {
    __BUILD_IS_BUILD__: JSON.stringify(ENV_WEB_SERVER.IS_BUILD),
  ...
  plugins: [
    !IS_TEST && ENV_WEB_SERVER.NODE_ENV === "development"
      ? devtools({
