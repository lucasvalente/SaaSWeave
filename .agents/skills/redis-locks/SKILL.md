---
name: redis-locks
description: Implementar travas distribuídas (Redlock) para evitar condições de corrida.
version: 1.0.0
...
## Purpose
Implementar travas distribuídas (Redlock) para evitar condições de corrida.

## When to use
Utilize esta skill sempre que trabalhar em tarefas, revisões ou alterações relacionadas a **Redis Locks** no ecossistema AUTUAX.
...
2. Aplicar as convenções e padrões estritos definidos para **Redis Locks**.
