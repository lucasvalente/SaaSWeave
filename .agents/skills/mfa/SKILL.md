---
name: mfa
description: Configurar fluxo seguro de verificação de autenticação de dois fatores (TOTP/SMS).
version: 1.0.0
...
# Multi-Factor Authentication Skill

## Purpose
Configurar fluxo seguro de verificação de autenticação de dois fatores (TOTP/SMS).

## When to use
Utilize esta skill sempre que trabalhar em tarefas, revisões ou alterações relacionadas a **Multi-Factor Authentication** no ecossistema AUTUAX.
...
2. Aplicar as convenções e padrões estritos definidos para **Multi-Factor Authentication**.
