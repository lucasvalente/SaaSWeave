...
description: Testar proteções contra injeção SQL, XSS, quebra de autorização e bypass de sessões.
version: 1.0.0
...
## Purpose
Testar proteções contra injeção SQL, XSS, quebra de autorização e bypass de sessões.

## When to use
Utilize esta skill sempre que trabalhar em tarefas, revisões ou alterações relacionadas a **Security Testing** no ecossistema AUTUAX.
...
2. Aplicar as convenções e padrões estritos definidos para **Security Testing**.
